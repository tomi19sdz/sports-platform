...
def trigger_fetch(request):
    ...
    if token != 'moje-tajne-haslo-123': return HttpResponse("Brak dostępu", status=403)
        ...
        if response.status_code == 200:
            ...
            for m in matches:
                ...
                if home_team and away_team and home_team.get('name') and away_team.get('name'):
                    ...
                    Match.objects.update_or_create(home_team=home, away_team=away, match_date=match_date, defaults={'home_logo': home_team.get('crest', ''), 'away_logo': away_team.get('crest', '')})
