          ...
          upcomingMatches.map(([date, matches]) => (
            <div key={date} className="mb-12">
              ...
              <div className="grid gap-4">
                {matches.map((match) => (
                  <Link key={match.id} href={`/match/${match.id}`} className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 hover:border-emerald-500/50 hover:bg-slate-800/60 hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row items-center justify-between group shadow-lg shadow-black/20">
