              ...
              <div className="grid gap-4">
                {matches.map((match) => (
                  <Link key={match.id} href={`/match/${match.id}`} className="relative bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 hover:border-emerald-500/50 hover:bg-slate-800/60 hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row items-center justify-between group shadow-lg shadow-black/20">
                    ...
                    <div className="w-full sm:absolute sm:left-6 sm:top-1/2 sm:-translate-y-1/2 sm:w-[20%] text-center sm:text-left mb-4 sm:mb-0 pointer-events-none">
                      <span className="text-[10px] sm:text-xs text-slate-500 group-hover:text-emerald-500/80 transition-colors font-bold uppercase tracking-widest block truncate" title={match.league}>
                        {match.league}
                      </span>
                    </div>
                    {/* ------------------------------------ */}
